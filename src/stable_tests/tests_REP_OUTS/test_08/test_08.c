...
static void rep_outs_test_08() {
	asm volatile (  "mov $0x3f8, %%edx \n" /* serial port COM 1 (emulated by ramooflax) */
					"mov $0x400000, %%esi   \n" /* address of buffer */
...
static int test_test_08()
{
	...
	__pg_set_entry( &(pTb.ptb[0]), PG_RW, page_number((uint32_t)page_a) );
...
test_t test_08 = {
	.name = "test 08: rep outs com1 in ring 3 from a kernel page",
	.desc = "Test the behavior of VMM with a 'REP OUTS COM1' instruction"
	        "performed in ring 3 which uses data of a kernel page."
	        "VMM should raise a #PF",
	.init = init_test_08,
	.test = test_test_08,
	.fini = fini_test_08,
};

DECLARE_TEST(test_08)
