...
void rep_outs() {
	/****************** This must cause a PF because CPL=3 and page rights are 0 ********************/
...
static void enter_r3() {
	...
	if (flag == 0) {
		printf_log(" NOK - ring 3 : rep outsb done without #PF\n");
	} else {
		printf_log(" OK - ring 3 : rep outsb done with a #PF\n");
	}
...
static int test_test_02()
{
	...
	__pg_set_entry( &(pTb.ptb[0]), PG_RW|PG_USR, page_number((uint32_t)page_b) );		/************ Bad rights *************/
	...
	set_desc_addr_type_limit(gdt[6], 0, 3, DS_addr, (1*PAGE_SIZE-1)>>12); // data RO pvl=0
	set_gdtr(gdtr);
	...
	*r3esp   = (uint32_t)enter_r3;
	leave_r0();	                                         /************** leave ring 0 to execute REP OUTS in ring 3 **********/

   return 0;
}

static int fini_test_02() {
	// Restore
	set_ds(get_kgdt_sel(4));
	disable_paging();
	set_cr3( __cr3 );
...
test_t test_02 = {
	.name = "test 02: 'rep outs' with bad access rights",
