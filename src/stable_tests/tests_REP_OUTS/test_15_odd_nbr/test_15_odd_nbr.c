...
static int init_test_15_odd_nbr() {
	int i;
...
static int test_test_15_odd_nbr() {
	int i;
	uint32_t port;
	// Test
	...
	for(i=IO_PORT_MIN; i<IO_PORT_MAX; i++) {
		...
		if(i==0x92 			// Vmw : reboot 											Ram : ps2 fast reset !				VBox : reboot
		|| i==0x2fb			// Vmw : nothing (but uart com2 stopped)					Ram : unsupported uart operation ! 	VBox : nothing
		|| i==0x3fd			// Vmw : nothing											Ram : unsupported uart operation !	VBox : nothing
		|| i==0x3fe			// Vmw : nothing											Ram : unsupported uart operation !	VBox : nothing
		|| i==0x1042		// Vmw : VERIFY bora/devices/chipset/piix4SM.c:437			Ram : no control (vmware crash)		VBox : nothing
...
static int restore_test_15_odd_nbr() {
	// Restore
	return 0;
}

test_t test_15_odd_nbr = {
	.name = "Test 15 : REP OUTS fuzzer - list of fails - value 'A'",
	...
	"- buffer size     : 1"
	"- buffer content  : 'A'" ,
	.init = init_test_15_odd_nbr,
	.test = test_test_15_odd_nbr,
	.fini = restore_test_15_odd_nbr,
};

DECLARE_TEST(test_15_odd_nbr)
