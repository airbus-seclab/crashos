...
static void rep_ins_test_07() {
	// REP INS instruction
   asm volatile (  "mov $0x3f8, %%edx \n" /* serial port COM 1 */
				   "mov $0x0, %%edi   \n" /* address of buffer */
				   "mov $0x20, %%ecx  \n" /* number of byte to write in the buffer */
				   "rep insb"
				   :::);
}
...
static void enter_r3()
{
	...
	if (flag == 0) {
		printf_log("NOK (no #PF)\n");
	} else {
		printf_log("OK\n");
	}
	set_es(get_ugdt_sel(6));
	rep_ins_test_07();
	set_es(get_ugdt_sel(4));
	if (flag == 0) {
			printf_log("NOK (no #PF)\n");
		} else {
			printf_log("OK\n");
...
static int test_7() {
   ...
   __pg_set_entry( &(pTb.ptb[0]), PG_RW|PG_USR, page_number((uint32_t)page_b) ); // Setup page table entries
    __pg_set_entry( &(pTb.ptb[1]), PG_RW, page_number((uint32_t)page_a) );
  // __pg_set_entry( &(pTb.ptb[1]), PG_USR, page_number((uint32_t)page_a) );
   ...
   set_desc_addr_type_limit(gdt[6], 3, 3, DS_addr, (1*PAGE_SIZE-1)>>12); // data RO pvl=0
   set_gdtr(gdtr);
   ...
   *r3esp   = (uint32_t)enter_r3;
   ...
   // REP OUTS instruction in ring 0
   set_ds(get_ugdt_sel(6));
   asm volatile (  "mov $0x3f8, %%edx \n" /* serial port COM 1 */
                   "mov $0x0, %%esi   \n" /* address of buffer */
                   "mov $0x20, %%ecx  \n" /* number of byte to read in the buffer */
   ...
   // REP INS instruction
     ...
     asm volatile (  "mov $0x3f8, %%edx \n" /* serial port COM 1 */
                     ...
                     "rep insb"
                     :::);
     set_es( get_kgdt_sel(4) );

  // while(1)      asm volatile("mov %cr3, %eax ; mov %eax, %cr3");

   return 0;
}

static int restore_test_07() {
   // Restore
   set_ds(get_kgdt_sel(4));
   disable_paging();
   set_cr3( __cr3 );
...
test_t test_07 = {
	.name = "test 07: 'rep outs' with an expand-down data segment",
	.desc = "Test behaviour of hypervisor when 'REP INS/OUTS COM1 r3'"
			"instruction is using a DS_r3 mapped on non consecutive"
			"physical pages and a pb in virtual pages access rights.",
	.init = init_test_07,
	.test = test_7,
	.fini = restore_test_07,
};


DECLARE_TEST(test_07)
